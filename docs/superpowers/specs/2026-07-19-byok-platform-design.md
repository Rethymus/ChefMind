...
- Retain GLM only as one OpenAI-compatible provider preset. Rename generic OpenAI-compatible code and user-facing copy so it is not presented as Qwen or GLM.
- Store Web API keys in memory by default; it must never persist a web API key in `localStorage`, IndexedDB, or compiled Vite environment variables. Startup deletes both legacy browser key records so users explicitly re-enter a key under the new policy.
...
- Settings explains the difference between desktop secure storage and web memory-only keys (lost on refresh). It never claims that a web key is securely persisted locally. Removing an API configuration also deletes the corresponding desktop keyring credential after confirmation.
