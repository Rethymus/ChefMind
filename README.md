...
[![AI Provider: BYOK](https://img.shields.io/badge/AI%20Provider-BYOK-2ea44f?style=flat-square)](#ai-配置)
...
- **从云端焦虑到本地掌控**：不注册账号、不代付模型额度；业务数据优先留在本机，适合个人使用、课程展示和桌面端封装。
...
- 凭据不会进入 `.env`、`VITE_*` 变量或构建产物。
- **Web / GitHub Pages**：Key 仅存在当前页面内存，刷新页面后需要重新填写。菜谱、收藏和购物清单保存于浏览器本地；请定期使用设置页的导出功能备份。
- **Tauri 桌面版**：Key 保存于操作系统凭据库，Rust 原生层直接请求供应商；Key 不写入 SQLite，也不会返回 WebView。
- 自定义 Base URL 必须是公共 HTTPS 地址。供应商还必须允许浏览器 CORS，GitHub Pages 才能直接调用。

本地演示或截图时可在设置页切换到 Mock 模式。

## GitHub Pages 演示版

仓库包含 GitHub Pages 部署工作流。启用仓库的 **Pages → GitHub Actions** 后，推送到 `main` 会发布静态 Web 版本。

Pages 版本是浏览器直连供应商的 BYOK 体验：ChefMind 和 GitHub 不接收、不托管 API Key；请求时所选供应商会收到该 Key。不要将任意供应商 Key 写入 GitHub Secret、`.env` 或 `VITE_*` 变量。若供应商不支持该 Pages 域名的 CORS，浏览器会阻止连接；静态站点无法绕过该限制。
...
- **桌面端**：Tauri 2, Rust
- **AI 接入**：OpenAI-compatible Chat Completions，BYOK、多供应商预设与自定义 Base URL
- **本地数据**：Web 使用 IndexedDB 与受控 localStorage；桌面端使用 SQLite 保存业务数据和系统凭据库保存 Key
