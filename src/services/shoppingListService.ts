...
class ShoppingListService {
  ...
  private generatePurchaseAdvice(name: string, amount: string, unit: string): string {
    ...
    if (unit === 'g' || unit === '克') {
      if (numAmount <= 100) {
        ...
        return `约${Math.ceil(numAmount / 100) * 100}g，建议购买500g装`
      } else {
      ...
      if (numAmount <= 3) {
        return `需要${Math.ceil(numAmount)}个`
      } else {
        return `需要${Math.ceil(numAmount)}个，建议多买几个备用`
      }
    } else if (unit === '片' || unit === '块') {
      return `需要${Math.ceil(numAmount)}${unit}`
    } else if (unit === '勺' || unit === '茶匙' || unit === '汤匙') {
      return '少量即可，家中常备'
    }
    
    return `需要${amount}${unit}`
  }
  ...
  private categorizeIngredient(name: string): string {
    // 蔬菜类
    const vegetables = ['土豆', '番茄', '洋葱', '大蒜', '生姜', '胡萝卜', '青椒', '白菜', '菠菜', '韭菜', '豆角', '茄子', '冬瓜', '南瓜', '莲藕', '山药', '玉米', '蘑菇', '木耳', '银耳', '豆芽', '芹菜', '黄瓜', '西红柿']
    
    // 肉类
    const meats = ['猪肉', '牛肉', '鸡肉', '鸭肉', '鱼', '虾', '蟹', '鸡蛋', '腊肉', '香肠', '培根', '羊肉', '排骨', '鸡翅', '鱼片']
    
    // 主食类
    const staples = ['大米', '面条', '面粉', '馒头', '面包', '红薯', '小米', '燕麦', '意大利面', '河粉', '米粉', '饺子皮']
    
    // 调味料
    const seasonings = ['盐', '生抽', '老抽', '料酒', '白糖', '香油', '胡椒粉', '花椒', '八角', '桂皮', '香叶', '蚝油', '醋', '辣椒油', '味精', '鸡精', '孜然', '五香粉']
  ...
  async addCategory(category: string): Promise<string[]> {
    ...
    try {
      // 从本地存储获取分类
      const savedCategories = localStorage.getItem(this.CATEGORIES_KEY)
      const categories: string[] = savedCategories ? JSON.parse(savedCategories) : ['食材', '调味料', '厨具', '其他']
