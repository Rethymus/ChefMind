import { ref } from 'vue'
import { getRandomImage, getImageByIngredients, getImageByCookingMethod, getSmartRecipeImage } from '@/utils/imageUtils'
...
请以JSON格式返回，格式如下：
{
      ...
      try {
        ...
        if (Array.isArray(parsedResponse)) {
          return parsedResponse.map((recipe, index) => ({
            ...
            image: recipe.image || getSmartRecipeImage(recipe.name || '食谱')
          }))
        }
   ...
   * 生成单个AI食谱
   */
  const generateAIRecipe = (params: RecipeGenerateParams, cookingMethod: string, index: number): Recipe => {
    ...
    let image = ''
    if (cookingMethod) {
      image = getImageByCookingMethod(cookingMethod)
    } else if (mainIngredients.length > 0) {
      image = getImageByIngredients(mainIngredients)
    } else {
  ...
  const searchRecipes = async (query: string): Promise<Recipe[]> => {
    ...
    try {
      ...
      if (savedRecipes) {
        ...
        localResults = parsedRecipes.filter((recipe: Recipe) => 
        ...
        try {
          ...
          if (Array.isArray(aiResults)) {
            const enhancedResults = aiResults.map((recipe, index) => ({
              ...
              image: getSmartRecipeImage(recipe.name || '食谱')
            }))
