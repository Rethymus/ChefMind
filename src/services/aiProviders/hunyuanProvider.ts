...
export class HunyuanProvider implements BaseAIProvider {
  private readonly apiKey: string
  private readonly baseURL: string
  private readonly model: string

  constructor(apiKey?: string, baseURL: string = 'https://hunyuan.tencentcloudapi.com') {
    this.apiKey = apiKey || import.meta.env.VITE_HUNYUAN_API_KEY || ''
    this.baseURL = baseURL || 'https://hunyuan.tencentcloudapi.com'
    this.model = import.meta.env.VITE_HUNYUAN_MODEL || 'hunyuan-lite'
  }

  private async callHunyuan(prompt: string, options?: { maxTokens?: number; temperature?: number }): Promise<string> {
    if (!this.apiKey) {
      throw new Error('Hunyuan API密钥未配置')
    }

    try {
      // 腾讯云Hunyuan使用签名认证，这里简化处理
      ...
      if (!response.ok) {
        ...
        throw new Error(`Hunyuan API错误: ${response.status} - ${error}`)
      }

      const data = await response.json()
      return data.Response.Choices[0].Message.Content
    } catch (error) {
      console.error('Hunyuan API调用失败:', error)
      throw error
    }
  }
  ...
  private parseJsonResponse<T>(response: string): T {
    try {
      ...
      console.error('解析Hunyuan JSON响应失败:', error)
  ...
  async generateRecipe(ingredientsOrParams: string[] | RecipeGenerationParams, preferences?: UserPreferences): Promise<Recipe> {
    try {
      console.log('🚀 Hunyuan生成食谱开始，参数:', JSON.stringify(ingredientsOrParams, null, 2))
      ...
      const response = await this.callHunyuan(prompt, {
        maxTokens: 2000,
        temperature: 0.7
      })
      ...
      console.log('✅ Hunyuan食谱生成成功:', recipe.title)
      return recipe
    } catch (error) {
  ...
  private buildRecipeFromResult(recipeResult: Partial<Recipe>, params: RecipeGenerationParams): Recipe {
    ...
    return {
      id: 'hunyuan-' + Date.now() + '-' + Math.random().toString(36).substring(2, 9),
      title,
      name: title,
  ...
  private createFallbackRecipe(params: RecipeGenerationParams): Recipe {
    ...
    return {
      id: 'hunyuan-fallback-' + Date.now() + '-' + Math.random().toString(36).substring(2, 9),
      title,
      name: title,
...
`
      ...
      console.error('Hunyuan验证食材失败:', error)
      return {
        isValid: true,
...
`
      ...
      console.error('Hunyuan分析食材失败:', error)
      return {
        name: '未知食材',
        confidence: 0.5,
        category: '其他',
...
`
      ...
      console.error('Hunyuan分析营养成分失败:', error)
      return {
        calories: 0,
        protein: 0,
        carbs: 0,
...
`
      ...
      console.error('Hunyuan获取个性化推荐失败:', error)
      return []
    }
  }
...
`
      ...
      console.error('Hunyuan获取烹饪指导失败:', error)
      return {
        guidance: '按照步骤进行操作',
        tips: ['注意火候', '及时调味'],
        nextStep: '继续下一步',
...
export default new HunyuanProvider()