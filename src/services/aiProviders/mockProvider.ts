...
export class MockAIProvider implements BaseAIProvider {
  async generateRecipe(params: RecipeGenerationParams): Promise<Recipe> {
    ...
    let autoCompletedIngredients: string[] = []
    ...
    // 根据参数生成模拟食谱
    const recipe: Recipe = {
      title: this.generateRecipeTitle(params.ingredients),
      description: `这是一道使用${params.ingredients.join('、')}制作的美味食谱，适合${params.servings || 4}人食用。`,
      ingredients: ingredients,
      instructions: this.generateInstructions(params.ingredients, params.cookingMethods, params.noMethodRestriction),
      cookingTime: params.cookingTime || '30分钟内',
      servings: params.servings || 4,
      difficulty: params.difficulty || '中等',
      cookingMethods: params.noMethodRestriction ? this.getRandomCookingMethods() : (params.cookingMethods || ['炒']),
  ...
  private generateRecipeTitle(ingredients: string[]): string {
    ...
    const cookingStyles = ['香煎', '爆炒', '清蒸', '红烧', '糖醋', '麻辣', '五香', '酱爆', '干锅', '香辣']
  ...
  private generateInstructions(ingredients: string[], cookingMethods: string[] = [], noMethodRestriction: boolean = false): string[] {
