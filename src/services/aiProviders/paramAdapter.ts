...
export class ParamAdapter {
  ...
  static toRecipeGenerationParams(
    ...
    if (ingredientsOrParams && typeof ingredientsOrParams === 'object' && ingredientsOrParams.ingredients) {
      return {
        ingredients: ingredientsOrParams.ingredients || [],
        cookingMethods: ingredientsOrParams.cookingMethods || ingredientsOrParams.cookingMethod ? [ingredientsOrParams.cookingMethod].filter(Boolean) : [],
        noMethodRestriction: false,
        kitchenware: ingredientsOrParams.kitchenware || [],
        dietaryRestrictions: ingredientsOrParams.dietaryRestrictions || preferences?.dietaryRestrictions || [],
        ...
        flavorPreferences: ingredientsOrParams.flavorPreferences || preferences?.flavorPreferences || [],
        spiceLevel: isSearchMode ? 'none' : (ingredientsOrParams.spiceLevel || preferences?.spiceLevel || 'medium'),
        sweetnessLevel: isSearchMode ? 'none' : (ingredientsOrParams.sweetnessLevel || preferences?.sweetnessLevel || 'medium'),
   ...
   */
  static fromPreferences(ingredients: string[], preferences: UserPreferences): RecipeGenerationParams {
    return {
      ingredients,
      cookingMethods: [],
      noMethodRestriction: false,
