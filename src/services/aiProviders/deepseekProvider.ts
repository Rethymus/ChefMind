...
export class DeepSeekProvider implements BaseAIProvider {
  ...
  private async callDeepSeek(prompt: string, options?: { maxTokens?: number; temperature?: number }): Promise<string> {
    if (!this.apiKey) {
  ...
  private parseJsonResponse<T>(response: string): T {
    try {
      ...
      console.error('解析DeepSeek JSON响应失败:', error)
  ...
  async generateRecipe(ingredientsOrParams: string[] | RecipeGenerationParams, preferences?: UserPreferences): Promise<Recipe> {
    try {
      console.log('🚀 DeepSeek生成食谱开始，参数:', JSON.stringify(ingredientsOrParams, null, 2))
      ...
      console.log('✅ DeepSeek食谱生成成功:', recipe.title)
      return recipe
    } catch (error) {
      console.error('DeepSeek生成食谱失败:', error)
  ...
  private buildRecipeFromResult(recipeResult: Partial<Recipe>, params: RecipeGenerationParams): Recipe {
    ...
    return {
      id: 'deepseek-' + Date.now() + '-' + Math.random().toString(36).substring(2, 9),
      title,
      name: title,
  ...
  private createFallbackRecipe(params: RecipeGenerationParams): Recipe {
    ...
    return {
      id: 'deepseek-fallback-' + Date.now() + '-' + Math.random().toString(36).substring(2, 9),
      title,
      name: title,
...
`
      ...
      console.error('DeepSeek验证食材失败:', error)
      return {
        isValid: true,
...
`
      ...
      console.error('DeepSeek分析食材失败:', error)
      return {
        name: '未知食材',
        confidence: 0.5,
        category: '其他',
...
`
      ...
      console.error('DeepSeek分析营养成分失败:', error)
      return {
        calories: 0,
        protein: 0,
        carbs: 0,
...
`
      ...
      console.error('DeepSeek获取个性化推荐失败:', error)
      return []
    }
  }
...
`
      ...
      console.error('DeepSeek获取烹饪指导失败:', error)
      return {
        guidance: '按照步骤进行操作',
        tips: ['注意火候', '及时调味'],
        nextStep: '继续下一步',
