...
class MemoryStorage {
  ...
  // 收藏相关
  async addFavorite(sessionId: string, recipeId: number, recipeTitle: string, recipeImage?: string) {
...
export class UserDatabaseService {
   ...
   * 获取或创建用户会话
   */
  async getCurrentSession(sessionId?: string): Promise<{ sessionId: string; preferences: Record<string, unknown> }> {
   ...
   * 更新用户偏好设置
   */
  async updatePreferences(sessionId: string, preferences: Record<string, unknown>): Promise<boolean> {
    try {
      if (isNode && User) {
        // Node.js 环境：使用 SQLite
...
export class FavoriteDatabaseService {
   ...
   * 添加收藏
   */
  async addFavorite(sessionId: string, recipeId: number, recipeTitle: string, recipeImage?: string): Promise<any> {
    try {
      if (isNode && Favorite) {
        // Node.js 环境：使用 SQLite
   ...
   * 获取用户收藏列表
   */
  async getUserFavorites(sessionId: string, limit: number = 50, offset: number = 0): Promise<any[]> {
    try {
      if (isNode && Favorite) {
        // Node.js 环境：使用 SQLite
