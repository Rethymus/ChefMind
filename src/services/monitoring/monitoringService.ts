...
export class MonitoringService {
  ...
  getUsageAnalytics(timeRange: number = 24 * 60 * 60 * 1000): UsageAnalytics {
    ...
    // 筛选时间范围内的指标
    const filterByTime = (metrics: MetricData[]) => metrics.filter(m => m.timestamp >= startTime)
...
按类型分布:
${Object.entries(analytics.storageUsage.byType)
  .map(([type, usage]) => `  ${type}: ${usage}MB`)
  .join('\n')}
    `.trim()
  }
  ...
  private estimateActiveUsers(): number {
    ...
    const uniqueUsers = new Set(recentRequests.map(m => m.tags?.userId || 'anonymous'))
