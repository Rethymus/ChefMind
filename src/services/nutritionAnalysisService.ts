...
export class NutritionAnalysisService {
   ...
   * 计算当前营养摄入量（通过AI分析）
   */
  private async calculateCurrentIntake(meals: UserProfile['meals']): Promise<NutritionAnalysisResult['currentIntake']> {
    ...
    try {
      // 构建AI分析提示
      const mealDescriptions = meals.map(meal => {
        const foodList = meal.foods.map(food => `${food.name} ${food.amount}${food.unit}`).join('、')
