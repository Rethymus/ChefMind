    ...
    >
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="活动水平" prop="activityLevel">
              <el-select v-model="formData.activityLevel" placeholder="请选择活动水平" style="width: 100%">
      ...
      <div class="form-section">
        ...
        <div v-else class="meals-list">
          <div 
            v-for="(meal, mealIndex) in formData.meals" 
            :key="mealIndex"
            class="meal-card"
          >
...
const formRules: FormRules = {
  ...
  healthGoals: [
    { type: 'array', min: 1, message: '请至少选择一个健康目标', trigger: 'change' }
  ]
}
...
const confirmAddMeal = () => {
  ...
  const timeStr = newMeal.time ? new Date(newMeal.time).toLocaleTimeString('zh-CN', { 
    hour12: false, 
    hour: '2-digit', 
    minute: '2-digit' 
  }) : ''
