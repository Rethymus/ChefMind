  ...
  if (showCandidates.value && candidates.value.length > 0) {
    switch (event.key) {
      ...
      case 'Enter':
        ...
        break
      case 'ArrowUp':
        selectedIndex.value = selectedIndex.value > 0 ? selectedIndex.value - 1 : candidates.value.length - 1
        event.preventDefault()
        break
      case 'ArrowDown':
        selectedIndex.value = selectedIndex.value < candidates.value.length - 1 ? selectedIndex.value + 1 : 0
        event.preventDefault()
        break
      case '1':
      case '2':
...
const selectCandidate = (index: number) => {
  if (index >= 0 && index < candidates.value.length) {
    ...
    if (input) {
      ...
      const newValue = value.slice(0, start - currentPinyin.value.length) + candidate.text + value.slice(start)
