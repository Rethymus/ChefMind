<template>
  <div class="cooking-method-selection">
    ...
    <div class="tips-section">
      <el-alert title="烹饪小贴士" type="info" :closable="false" show-icon>
        <template #default>
          <ul class="tips-list">
...
<script setup lang="ts">
  ...
  const generateRecommendations = () => {
    ...
    // 简单的推荐逻辑
    const availableMethods = cookingMethods.filter(method => !isSelected(method))
    ...
    if (props.selectedMethods.length === 0) {
      ...
      // 根据已选方法推荐互补方法
      const hasQuickMethod = props.selectedMethods.some(m => ['炒', '煎', '炸'].includes(m.name))
      const hasSlowMethod = props.selectedMethods.some(m => ['炖', '焖', '煮'].includes(m.name))
