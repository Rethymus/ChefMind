<template>
  <div class="constraint-selection">
    ...
    <div class="smart-suggestions">
      <el-alert title="智能建议" type="success" :closable="false" show-icon>
        <template #default>
          <div class="suggestions-content">
            <p v-if="!hasAnySelection">请选择至少一个约束条件，AI将为你生成更精准的菜谱推荐</p>
            <div v-else>
              <p>根据你的选择，AI建议：</p>
  ...
  const getSelectedLabel = (type: keyof Constraints): string => {
    ...
    if (type === 'difficulty') {
      if (props.constraints.difficulty === null) {
        ...
        props.constraints.difficulty !== undefined &&
        props.constraints.difficulty !== null
      ) {
        const difficultyMap: Record<number, string> = {
          1: 'easy',
          3: 'medium',
    ...
    const option = optionsMap[type as keyof typeof optionsMap]?.find(
