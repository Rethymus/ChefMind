  ...
  <div class="ingredient-recognition">
    ...
    <div v-if="recognizedIngredients.length > 0" class="recognition-results">
      ...
      <div class="results-actions">
        <button class="action-button full-width" @click="addAllIngredients">
          添加全部
        </button>
      </div>
    </div>
...
// 识别食材
const recognizeIngredients = async (file: File, _source: string) => {
  try {
    ...
    // 使用AI服务进行食材识别
    const { useUserService } = await import('@/services/userService')
    ...
    const prompt = `请识别图片中的食材，返回JSON格式的结果：
[{"name": "食材名称", "confidence": 置信度(0-1之间的小数)}]
只返回能明确识别的食材，置信度应该真实反映识别的准确性。最多返回5种食材。`
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        recognizedIngredients.value = results.filter((item: any) => 
          item.name && typeof item.confidence === 'number' && item.confidence > 0.5
        )
      } else {
