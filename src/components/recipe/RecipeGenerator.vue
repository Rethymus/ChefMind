...
import { useRecipeService, type Recipe, type RecipeGenerateParams } from '@/services/recipeService'
...
const ingredientCategories = ref([
  { 
    name: '蔬菜类', 
    items: ['白菜', '萝卜', '胡萝卜', '土豆', '番茄', '黄瓜', '茄子', '豆角', '韭菜', '菠菜', '生菜', '芹菜', '洋葱', '青椒', '冬瓜', '南瓜', '丝瓜', '苦瓜', '西葫芦', '莲藕', '山药', '豆芽']
  },
  { 
    name: '肉类', 
    items: ['猪肉', '牛肉', '羊肉', '鸡肉', '鸭肉', '鹅肉', '火鸡', '兔肉', '猪排骨', '牛排', '鸡翅', '鸡腿', '鸡胸肉', '猪肚', '牛肚', '猪肝', '牛肝', '羊肝']
  },
  { 
    name: '海鲜类', 
    items: ['鱼', '虾', '蟹', '贝类', '鱿鱼', '章鱼', '海参', '龙虾', '三文鱼', '金枪鱼', '带鱼', '鲈鱼', '黄鱼', '鲤鱼', '草鱼', '鲫鱼']
  },
  { 
    name: '豆制品', 
    items: ['豆腐', '豆干', '豆皮', '腐竹', '豆腐脑', '豆腐丝', '豆腐泡', '豆腐乳', '豆浆', '豆芽']
  },
  { 
    name: '主食类', 
    items: ['米饭', '面条', '馒头', '包子', '饺子', '面包', '粥', '意大利面', '通心粉', '河粉', '米粉', '方便面', '挂面', '拉面', '刀削面', '炒饭', '炒面']
  }
])
...
const generateRecipes = async () => {
  ...
  try {
    ...
    const params: RecipeGenerateParams = {
      ...
      cookingMethods: noMethodRestriction.value ? [] : selectedMethodObjects.value.map(m => m.name),
...
const getRecipeImageStyle = (recipe: Recipe) => {
  return {
    backgroundImage: `url(${getSmartRecipeImage(recipe)})`
  }
}
