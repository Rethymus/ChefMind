      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <div class="form-row">
            ...
            <div class="form-group">
              <label>年龄</label>
              <input type="number" v-model="profileForm.age" class="form-input" min="1" max="120" />
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
...
<script setup lang="ts">
  ...
  const totalCalories = computed(() => {
    ...
    return Math.round(
      recipeStore.selectedIngredients.reduce((total, ingredient) => {
  ...
  const healthSuggestions = computed((): HealthSuggestion[] => {
    ...
    // 蔬菜建议
    const vegetableCount = recipeStore.selectedIngredients.filter(
      ing => ing.category === '蔬菜'
    ).length
    if (vegetableCount < 3) {
      suggestions.push({
