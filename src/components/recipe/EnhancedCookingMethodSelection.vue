...
<script setup lang="ts">
  ...
  // import type { CookingMethod } from '@/types/recipe' // 暂时注释未使用的类型
  ...
  watch(
    ...
    newVal => {
      if (Array.isArray(newVal)) {
        selectedMethodsLocal.value = [...(newVal as string[])]
      }
    },
    { immediate: true }
  )
  ...
  onMounted(() => {
    ...
    if (Array.isArray(props.selectedMethods)) {
      selectedMethodsLocal.value = [...(props.selectedMethods as string[])]
    }
  })
</script>
