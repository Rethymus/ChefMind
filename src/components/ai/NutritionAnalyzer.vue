          ...
          <div class="nutrition-breakdown">
            <div class="nutrient-item" v-for="(value, key) in recipeNutrition" :key="key">
              <div class="nutrient-info">
                <span class="nutrient-name">{{ getNutrientName(key) }}</span>
                <span class="nutrient-value">{{ formatNutrientValue(key, value) }}</span>
              </div>
              <div class="nutrient-bar">
                <el-progress
                  :percentage="getNutrientPercentage(key, value)"
...
import { aiService, type RecipeRecommendation, type HealthAnalysis } from '@/services/aiService'
...
const nutritionAnalysis = ref<HealthAnalysis>()
...
const analyzeNutrition = async () => {
  ...
  try {
    const analysis = await aiService.analyzeNutrition(currentRecipe.value, userProfile)
...
const drawNutritionChart = () => {
  ...
  // 销毁现有图表
  Chart.getChart(nutritionChartRef.value)?.destroy()
  ...
  new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: [
          recipeNutrition.value.protein * 4, // 蛋白质热量
          recipeNutrition.value.carbs * 4,   // 碳水热量
          recipeNutrition.value.fat * 9      // 脂肪热量
        ],
        backgroundColor: [
          '#67c23a', // 绿色 - 蛋白质
          '#e6a23c', // 橙色 - 碳水
...
const calculateRecommendedPortion = () => {
  ...
  const recipeCalories = currentRecipe.value.nutrition.calories
