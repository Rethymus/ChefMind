      ...
      <div v-else class="cooking-progress">
        ...
        <div class="progress-header">
          ...
          <el-progress
            ...
            :show-text="false"
          />
          <div class="progress-text">步骤 {{ currentStep }} / {{ totalSteps }}</div>
        </div>
            ...
            <el-collapse-item title="💡 烹饪小贴士" name="tips">
              <div class="tips-content">
                ...
                <div v-if="currentStepData?.commonMistakes.length" class="common-mistakes">
                  <h6>常见错误:</h6>
                  <ul>
                    <li v-for="mistake in currentStepData.commonMistakes" :key="mistake">
                      {{ mistake }}
                    </li>
                  </ul>
                </div>
        ...
        <div class="control-buttons">
          <el-button v-if="currentStep > 1" @click="previousStep" :disabled="isProcessing">
          ...
          <el-button v-if="stepTimer.isActive" @click="stopTimer" type="danger">
          ...
          <el-button v-else @click="completeCooking" :disabled="isProcessing" type="success">
...
<script setup lang="ts">
  ...
  onMounted(async () => {
    ...
    try {
      ...
      const recipes = await aiService.getPersonalizedRecommendations(
        userHistory,
        userPreferences,
        5
      )
