...
export class RecipeModel {
   ...
   * 创建食谱
   */
  async create(recipeData: Omit<RecipeDocument, 'id' | 'createdAt' | 'updatedAt'>): Promise<RecipeDocument> {
    const data = {
  ...
  async update(id: number, updates: Partial<RecipeDocument>): Promise<RecipeDocument | null> {
    ...
    if (updates.nutritionInfo !== undefined) data.nutrition_info = JSON.stringify(updates.nutritionInfo)
    if (updates.imageUrl !== undefined) data.image_url = updates.imageUrl
    if (updates.cookingMethods !== undefined) data.cooking_methods = JSON.stringify(updates.cookingMethods)
