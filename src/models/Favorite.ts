...
export class FavoriteModel {
   ...
   * 添加收藏
   */
  async addFavorite(sessionId: string, recipeId: number, recipeTitle: string, recipeImage?: string): Promise<FavoriteDocument> {
    const data = {
   ...
   * 获取用户的所有收藏
   */
  async getUserFavorites(sessionId: string, limit: number = 50, offset: number = 0): Promise<FavoriteDocument[]> {
