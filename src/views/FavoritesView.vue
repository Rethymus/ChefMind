<template>
  <div class="favorites-view" style="min-height: 100vh; background: white; padding: 20px">
    ...
    <div
      ...
      style="background: yellow; min-height: 80vh; padding: 20px; border: 3px solid red"
    >
      ...
      <div
        ...
        class="empty-state"
        style="background: lightcoral; padding: 40px; border: 3px solid darkred"
      >
        ...
        <div v-else class="recipes-list">
          <div v-for="recipe in filteredRecipes" :key="recipe.id" class="recipe-list-item">
            ...
            <div class="recipe-list-cover">
              <div
                class="recipe-list-svg"
                v-html="generateListRecipeSvg(recipe.name || recipe.title)"
              ></div>
            </div>
...
<script setup lang="ts">
  ...
  const loadSavedRecipes = async () => {
    ...
    try {
      console.log('🔄 开始加载收藏食谱...')

      // 检查 Tauri API 是否可用
      if (!window.__TAURI__) {
        console.warn('⚠️ Tauri API 不可用，使用模拟数据')
        savedRecipes.value = []
        return
      }

      // 使用 Session 服务获取 sessionId
      const { sessionService } = await import('@/services/sessionService')
      const sessionId = sessionService.getCurrentSessionId()
      console.log('📋 使用会话ID:', sessionId)

      // 使用 Tauri 数据库服务获取收藏
      const { tauriDatabaseService } = await import('@/services/tauriDatabaseService')
      console.log('🔧 数据库服务已加载')

      // 先测试数据库连接
      const dbStatus = await tauriDatabaseService.getDatabaseStatus()
      console.log('🔍 数据库状态:', dbStatus)

      if (!dbStatus.connected) {
        console.error('❌ 数据库连接失败:', dbStatus.error)
        savedRecipes.value = []
        return
      }
      ...
      if (!Array.isArray(favorites)) {
        console.error('❌ 收藏数据格式错误:', favorites)
        savedRecipes.value = []
        return
      }

      // 转换为食谱数据格式
      const fullRecipes = favorites.map((fav, index) => {
        try {
          return {
            id: fav.recipe_id?.toString() || `unknown_${index}`,
            name: fav.recipe_title || fav.title || '未命名菜谱',
            title: fav.recipe_title || fav.title || '未命名菜谱',
            description: fav.description || '',
            ingredients: fav.ingredients ? (typeof fav.ingredients === 'string' ? JSON.parse(fav.ingredients) : fav.ingredients) : [],
            steps: fav.instructions ? (typeof fav.instructions === 'string' ? JSON.parse(fav.instructions) : fav.instructions) : [],
            cookingTime: fav.cooking_time || '30',
            difficulty: fav.difficulty || '中等',
            servings: fav.servings || 2,
            rating: fav.average_rating || 0,
            tags: fav.tags ? (typeof fav.tags === 'string' ? JSON.parse(fav.tags) : fav.tags) : [],
            image: fav.recipe_image || fav.image_url || '',
            cookingMethods: [],
            nutrition: fav.nutrition_info ? (typeof fav.nutrition_info === 'string' ? JSON.parse(fav.nutrition_info) : fav.nutrition_info) : {},
            createdAt: fav.created_at,
          }
        } catch (parseError) {
          console.error(`❌ 解析第 ${index} 条收藏数据失败:`, parseError)
          return {
            id: `error_${index}`,
            name: '数据解析错误',
            title: '数据解析错误',
            description: '无法解析此收藏数据',
            ingredients: [],
            steps: [],
            cookingTime: '30',
            difficulty: '中等',
      ...
      console.log('🎯 最终食谱数据:', fullRecipes)
      savedRecipes.value = fullRecipes

      // 显示调试信息
      console.log(`✅ 成功加载 ${fullRecipes.length} 条收藏食谱`)
      ...
      // 尝试提供更有用的错误信息
      let errorMessage = '加载收藏失败'
      if (error.message?.includes('Tauri')) {
        errorMessage = 'Tauri API 不可用，请确保在 Tauri 应用中运行'
  ...
  const removeFromFavorites = async (recipe: Recipe) => {
    try {
      ...
      await tauriDatabaseService.removeFavorite(sessionId, parseInt(recipe.id))
